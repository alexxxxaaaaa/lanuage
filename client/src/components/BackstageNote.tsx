...
export function BackstageNote() {
  return (
    <article className="backstage-note">
      <header className="backstage-note-header">
        <h1>把团队默契装进 Claude Code —— 本周做的四件事</h1>
        <p className="backstage-note-meta">zyd · 2026-05-29</p>
        <p>
      ...
      <ul>
        ...
        <li>
          新建主题要在 <code>k-football</code> / <code>k-expert-admin</code> /{' '}
          <code>waiter-admin</code> 三个仓库分别建目录、填配置,半小时起步
        </li>
...
└── CLAUDE.md                  # ④ 项目快照(待办)`}</code>
      ...
      <p>
        <code>permissions.allow</code> 本质上就是项目级白名单。在里面的操作:
      </p>
      <ul>
  ...
  "permissions": {
    "allow": [
      ...
      <ol>
        <li>
          <strong>扫历史会话</strong>:统计你在本项目里点过哪些"允许"。
        </li>
        <li>
          <strong>聚类 + 排序</strong>:按频率排序,并过滤危险命令(<code>rm</code>,{' '}
          <code>sudo</code>, <code>git push --force</code>)。
        </li>
        <li>
          <strong>推荐合入 settings.json</strong>:最后给出推荐列表。
        </li>
      </ol>
      <pre>
        <code>{`建议加入 permissions.allow:
...
⚠ Bash(sudo:*)                    点了 3 次`}</code>
      ...
      <table className="backstage-note-table">
        <thead>
          <tr>
            <th>文件</th>
            <th>内容</th>
      ...
      <ol>
        <li>
          <strong>主题名格式校验</strong> —— 仅允许 <code>[a-z0-9-]</code>
        </li>
        <li><strong>目录冲突检查</strong> —— 避免覆盖已有主题。</li>
        <li>
          <strong>分支检查</strong> —— 禁止 release / master / main。
        </li>
        <li>
          <strong>路径配置检查</strong> —— 确认 <code>repo-paths.json</code> 存在且有效。
        </li>
        <li>
          <strong>不自动 rollback</strong> —— 失败不自动 cleanup,避免误删已编辑内容。
        </li>
      </ol>
...
→ /theme-image-pipeline`}</code></pre>
      ...
      <h3>核心思想</h3>
      <p>
        这个 slash command 的设计理念,其实和 Michael 那套 <code>/spec</code> 一样:
      </p>
      ...
      <p>今天团队确认:</p>
      <blockquote>
        以后统一走 <code>/auto-commit</code>,不再手工 <code>git commit</code>。
      </blockquote>
      ...
      <ol>
        <li>
          <strong>commit message 统一</strong> —— 不再中英混乱 / prefix 混乱 / 风格混乱。
        </li>
        <li><strong>提交前强制 review</strong> —— 提前挡掉低级错误。</li>
        <li>
          <strong>生产分支保护</strong> —— <code>release-v2</code> 误提交直接拦截。
        </li>
      </ol>
