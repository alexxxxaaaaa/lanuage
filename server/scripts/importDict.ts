...
async function exportD1() {
  ...
  const apply = [
    ...
    '#',
    '# 分片按顺序执行：第一片带 DELETE，先清空再整批插入。中途失败可以从',
    '# 失败那片继续（改下面的起始序号），前面已插入的行不会重复 —— 只要',
    '# 没有重跑第一片。',
    'set -euo pipefail',
    'cd "$(dirname "$0")/.."',
    '',
    `echo "先确保建表迁移已应用："`,
    `wrangler d1 execute word-sprint-db --remote --file=./prisma/migrations/${MIGRATION}/migration.sql || true`,
    '',
    ...files.map(
      (f) => `echo "→ ${f}"\nwrangler d1 execute word-sprint-db --remote --file=./d1_dict/${f}`,
    ),
    '',
    'echo "完成。"',
  ].join('\n')
